class Translator {
  ...
  _detectLanguage() {
    ...
    document.querySelector("#page_loading").classList = "hide";
  ...
  async load(lang) {
    lang = lang ? lang : this._options.defaultLanguage;
    ...
    var replace = element => {
      ...
      if (keys.length > 0 && keys.length !== properties.length) {
        ...
        pairs.forEach(pair => {
          ...
          var key_value = nullSafeSplit(element.getAttribute("data-value"), " ") || [];

          if (key_value.length > 0) {
            key_value = key_value[0].split("|");
            if (text.indexOf("%s") > -1) {
              for (let i = 0; i < key_value.length; i++) {
                text = text.replace("%s", key_value[i]);
              }
            }
          }
